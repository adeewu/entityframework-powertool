...
namespace Microsoft.DbContextPackage.Utilities
{
    ...
    public class EfTextTemplateHost : ITextTemplatingEngineHost
    {
        ...
        {
            var contextName = EntityContainer.Name.Replace("Context", "");
            var values = extension.Split(new char[] { '|' }, StringSplitOptions.RemoveEmptyEntries);
            if (values.Count() >= 1)
            {
                FileExtension = values[0];
            }
            if (values.Count() >= 2)
            {
                NameSpaceSuffix = string.Format(values[1], contextName);
            }
            if (values.Count() >= 3)
            {
                NameSuffix = string.Format(values[2], contextName);
            }
        }
